...
from game.scripting.collide_net_action import CollideNetAction
...
class SceneManager:
    ...
    COLLIDE_NET_ACTION = CollideNetAction(PHYSICS_SERVICE, AUDIO_SERVICE)
    ...
    def _add_lives(self, cast):
        ...
        text2 = Text(LIVES_FORMAT, FONT_FILE, FONT_SMALL, ALIGN_RIGHT)
        position2 = Point(HUD_MARGIN*8, HUD_MARGIN)
        label2 = Label(text2, position2)
        cast.add_actor(LIVES_GROUP, label2)
    ...
    def _add_stats(self, cast):
        ...
        stats = Stats(constants.RED)
        cast.add_actor(STATS_GROUP, stats)
        stats2 = Stats(constants.GREEN)
    ...
    def _add_racket(self, cast):
        ...
        racket = Racket(body, animation, constants.RED)
        ...
        racket2 = Racket(body2, animation2, constants.GREEN)
        cast.add_actor(RACKET_GROUP, racket2)

    def _add_net(self, cast):
        cast.clear_actors(NET_GROUP)

        x2 = CENTER_X 
        y2 = SCREEN_HEIGHT - RACKET_HEIGHT
        position = Point(x2, y2)
        size = Point(NET_WIDTH, NET_HEIGHT)
        velocity = Point(0, 0)
        body = Body(position, size, velocity)
        animation = Animation(NET_IMAGES, 1)
        racket = Racket(body, animation, constants.WHITE)
        cast.add_actor(NET_GROUP, racket)
    ...
    def _add_update_script(self, script):
        ...
        script.add_action(UPDATE, self.COLLIDE_NET_ACTION)
        script.add_action(UPDATE, self.MOVE_RACKET_ACTION)